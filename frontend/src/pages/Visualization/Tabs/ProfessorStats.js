...
const ProfessorStats = () => {
        ...
        .then((response) => {
          ...
          for (const key in profData) {
            const newDataset = {
              label: key,
              data: [
                profData[key].LectureDifficulty,
                profData[key].FinalDifficulty,
                profData[key].Workload,
                profData[key].LectureQuality,
                (profData[key].GPA * 5.0) / 4.3,
              ],
  ...
  return (
    <Box
      sx={{ diaplay: "flex", flexDirection: "column", alignItems: "center" }}
    >
